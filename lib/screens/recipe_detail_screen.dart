...
class _RecipeDetailScreenState extends State<RecipeDetailScreen> {
  ...
  Future<void> _callGemini() async {
    ...
    final prompt =
        '''
너는 TV 예능 프로그램 '냉장고를 부탁해'에 출연한 요리 연구가 '제미나이 쉐프'야. 
...
레시피 작성 조건: 
1. 한국어로 작성할 것. 
2. 요리 제목은 사용 재료를 나열하지 않고 사용할 주재료만 포함하여 자연스럽게 새로 작성할 것. 
...
6. 요리의 정체성을 유지하는 데 필요한 주재료(예: 고기, 해산물, 면 등)가 [냉장고 속 재료]에 존재하지 않는 경우, 역할이 유사한 대체 재료도 없다면 무리하게 레시피를 변형하지 말고 조건 9번을 따를 것. 
7. [냉장고 속 재료]나 기본 양념을 제외한 권장 재료는 재료와 조리법 사이에 '💡 OO를 추가하면 더욱 좋아요.' 형식으로 반드시 변형 레시피를 기준으로 팁 한 줄만 추가할 것. 
8. 레시피 형식은 반드시 설명, 인사말, 결론을 작성하지 않고 [주어진 레시피]와 동일하게 '제목 - 재료(팁) - 조리법' 순으로 형식을 지켜서 작성할 것.
