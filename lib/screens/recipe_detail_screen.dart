...
4. 기본 양념을 제외한 변형된 레시피의 재료 목록에 없는 재료는 요리 제목이나 조리법에 작성하지 말것.
5. 현실에 존재하지 않는 가상의 재료나 손질하는데 전문 자격이 필요한 위험한 재료(복어, 독버섯, 야생동물 고기 등)는 [냉장고 속 재료]에 있더라도 절대 사용하지 말것.
...
8. [냉장고 속 재료] 외의 재료는 기본 양념(소금, 설탕, 간장, 식용유, 참기름, 후추, 고춧가루, 된장, 고추장, 식초, 다진마늘, 마요네즈, 케첩, 밥, 물, 밀가루, 김치)만 추가할 것. 
9. 요리의 정체성을 유지하는 데 필요한 주재료(예: 고기, 해산물, 면 등)가 [냉장고 속 재료]에 존재하지 않는 경우, 역할이 유사한 대체 재료도 없다면 무리하게 레시피를 변형하지 말고 조건 12번을 따를 것. 
10. [냉장고 속 재료]나 기본 양념을 제외한 권장 재료는 재료와 조리법 사이에 '💡 OO를 추가하면 더욱 좋아요.' 형식으로 반드시 변형 레시피를 기준으로 팁 한 줄만 추가할 것. 
11. 레시피 형식은 반드시 설명, 인사말, 결론을 반드시 작성하지 않고 [주어진 레시피]와 동일하게 '제목 - 재료(팁) - 조리법' 순으로 형식을 지켜서 작성할 것.
12. [냉장고 속 재료]가 너무 부족해 요리 성립이 어렵다고 판단되면 기존 레시피 형식을 무시하고 아래 형식으로만 답변할 것:
